...
以下が失敗した場合、自動でメール通知が送られる(`.env.example`のSMTP_*/ALERT_EMAIL_TO参照)。
未設定の間は通知自体がスキップされるだけで、アプリ本体の動作には影響しない。

- `orders/paid` Webhookの記録・チケット付与・クーポン使用記録の失敗(お客様が支払い済みなのに
  チケットが付与されない、等の実害が出うるケース)
- ガチャ抽選処理自体の失敗、景品在庫切れによる抽選不能
- 抽選成立後のクーポン発行失敗(チケットは消費済みだが景品コードが未発行のケース)

Gmailを送信元にする場合は、Googleアカウントの「アプリパスワード」を発行してSMTP_PASSに設定する
(通常のログインパスワードは使えない)。Vercelの環境変数に追加後、再デプロイで反映される。

### Supabaseの無料プランは一定期間アクセスが無いと自動停止する(重要)

Supabaseの無料(Free)プランは、**API呼び出しが約1週間無いとプロジェクトが自動的に一時停止
(pause)される**。停止中はDBへの接続が一切できなくなり、購入・抽選・Webhookなど
アプリ全体が停止する(エラーにはなるが気付きにくい)。

本番で実際にお客様が使うアプリの場合は、**Supabaseを有料プラン(Pro以上)にアップグレード
することを強く推奨する**(自動停止が発生しなくなる)。無料プランのまま運用する場合は、
稼働状況を定期的に確認するか、簡単なヘルスチェック(定期的にAPIを叩く仕組み)を
別途用意すること。
